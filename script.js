...
function iniciarTransicao() {
    ...
    // 1. Remove COMPLETAMENTE a tela de boas-vindas
    const boasVindasSection = document.getElementById('boas-vindas');
    if (boasVindasSection) {
        boasVindasSection.style.opacity = '0';
        boasVindasSection.style.transition = 'opacity 0.5s ease';
        
        // Depois da animação, REMOVE do DOM para não voltar
        setTimeout(() => {
            boasVindasSection.style.display = 'none';
            // Impede que volte a aparecer
            boasVindasSection.style.visibility = 'hidden';
            boasVindasSection.style.position = 'absolute';
            boasVindasSection.style.zIndex = '-1000';
            boasVindasSection.style.pointerEvents = 'none';
        }, 500);
    }
    
    // 2. Mostra a transição
    const transicaoSection = document.getElementById('transicao');
    if (transicaoSection) {
        transicaoSection.style.display = 'flex';
        transicaoSection.style.visibility = 'visible';
        transicaoSection.style.opacity = '0';
        transicaoSection.style.position = 'fixed';
        transicaoSection.style.zIndex = '999';
        ...
        console.error("❌ Seção de transição não encontrada!");
        // Fallback: vai direto para o player de música
        setTimeout(() => {
            mostrarPlayerMusica();
        }, 1000);
    }
